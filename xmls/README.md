...
<widget>
	<api>
		...
		<prop name="mode" help="The current mode">
			<param name="mode" type="enum:my_widget_mode" help="The current mode to set as an enum"/>
		</prop>
...
- As `type`, any other widget's enums can be used in the following format: `enum:<widget_name>_<enum_name>`. For example `enum:lv_label_long_mode`.
...
In the XML normally widets and components are described.
By using a widget or component in the `<view>` of an XML file an instance of will be created.

However, besides widgets and components screens are also important to create a well structured UI.
...
The `<style>` tag can have `<variant>` child tags.
...
`<variant>`s can alter styles at runtime.
...
- font and image tags in path and name
...
```c
#if MY_COMPONENT_LIBRARY_SIZE == MY_COMPONENT_LIBRARY_SIZE_SMALL
	#define MY_WIDGET_PAD 4
#elif MY_COMPONENT_LIBRARY_SIZE == MY_COMPONENT_LIBRARY_SIZE_LARGE
...
The following section create a mapping between font names, and the path and various attributes.
...
Later the fonts can be referenced by its name (e.g. `<style name="style1" text_font="medium"`);
When exported to C `text_font="medium"` is reaplced by the actual value: `lv_style_set_text_font(&style1, &font_medium);`.
Or if it was a path TinyTTF or Freetype create function are also exported and the created font is used.

The tag name tells how to load the font:
- `bin`:
  - If `as_file="true"`: Convert the font file to `bin` (dee `lv_font_conv`)
    which can be loaded by `lv_binfont_create()`
...
For the sake of simplicity if `as_file="false"` in the preview the fonts will be loaded as files.
 `as_file="false"` affects only the C export.

If the UI is created from XML at runtime and a `global.xml` is parsed, the `as_file="false"` tags are skipped
because it's assumed that user creates the mapping by hand. It's because the XML is parser is not able
to map e.g. `lv_font_t my_font_24` to `name="my_font_24"`


Constants also can be used with fonts too.
...
An `<images>` section can be added to `global.xml` files.

This section desctibes how to map images with names.
The `<convert>` tag is optional.
...
`<file>` means that the image source is used as a file path: `lv_image_set_src(img, "path/to/file.png");`

`<data>` means that image is converted to a C array on export.
For the sake of simplicity in the preview the images are always loaded as files.
If the UI is created from XML at runtime and a `global.xml` is parsed, the `<data>` tags are skipped
because it's assumed that user creates the mapping by hand. It's because the XML is parser is not able
to map e.g. `lv_image_dsc_t my_logo` to `name="my_logo"`.

Constants also can be used with images too.
...
<button width="100px">
	<events trigger="clicked">
		...
		<load_screen name="subject1" value="Hello!"/>
		<update>
			<slider name="slider1" x="20" value="30"/>
			<obj name="panel1" add_style="red" remove_style="blue"/>
		</update>
	</events>
	<label text="Apply"/>
</button>
...
`<const_variant>`s can be used by constants to create variants that can be selected at compile time.

`<style_variant>`s can be used by styles only, to modify the styles at runtime.
The select the current dynamic variant a `<config.name>_variant` integer subject created which can be used to alter the styles if needed.
